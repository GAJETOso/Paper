/**
 * erp service — SAP/Oracle/Dynamics adapters behind one integration contract.
 *
...
const server = createServer((req, res) => {
  ...
  if (req.url === "/health") {
    res.writeHead(200).end(JSON.stringify({ status: "ok", service: "erp" }));
    return;
  }
  res.writeHead(404).end(JSON.stringify({ error: { code: "not_found", message: "Not found" } }));
});

server.listen(PORT, () => console.info("[erp] listening on :" + PORT));
