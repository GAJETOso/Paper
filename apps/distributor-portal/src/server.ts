/**
 * Distributor portal: tiered pricing, territory management, ordering, marketing assets.
 *
...
const server = createServer((req, res) => {
  ...
  if (req.url === "/health") {
    res.writeHead(200).end(JSON.stringify({ status: "ok", service: "distributor-portal" }));
    return;
  }
  ...
  if (req.url === "/api/v1/me") {
    ...
    if (!payload || !hasRole(payload.role, "DISTRIBUTOR")) {
      res.writeHead(401).end(
        JSON.stringify({
          error: { code: "unauthorized", message: "Valid DISTRIBUTOR token required" },
        }),
      );
      return;
    }
...
server.listen(PORT, () => console.info("[distributor-portal] listening on :" + PORT));
